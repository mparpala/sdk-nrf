...
#define NRF_CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE (256)
#define NRF_CRYPTO_EXAMPLE_AES_BLOCK_SIZE (16)

/* AES IV buffer */
static uint8_t m_iv[NRF_CRYPTO_EXAMPLE_AES_BLOCK_SIZE];
static uint8_t my_iv[NRF_CRYPTO_EXAMPLE_AES_BLOCK_SIZE] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07,0x00,0x00,0x00,0x00,0x00,0x00,0x00,0x00};
//static uint8_t my_iv[8] = {0x00,0x01,0x02,0x03,0x04,0x05,0x06,0x07};
...
{
	psa_status_t status;
	const uint8_t key_data[] = {0x00, 0x01, 0x02, 0x03, 0x04, 0x05, 0x06, 0x07, 0x08, 0x09, 0x0a, 0x0b, 0x0c, 0x0d, 0x0e, 0x0f};
...
int encrypt_ctr_aes(void)
{
	...
	//nonce = bytes.fromhex('0001020304050607')  # Static 64-bit nonce for CTR mode
	status = psa_cipher_set_iv(&operation, my_iv, sizeof(my_iv));
	if (status != PSA_SUCCESS) {
		LOG_INF("psa_cipher_set_iv failed! (Error: %d)", status);
		return APP_ERROR;
	}

#define ITERATIONS ( (16*1024*1024) / 256)
	...
	//LOG_INF("encrypted: %s", m_encrypted_text);
	...
	LOG_INF("Encryption successful! %d %d \r\n", ITERATIONS, i);
...
int main(void)
{
	...
	for(int i = 0; i < NRF_CRYPTO_EXAMPLE_AES_MAX_TEXT_SIZE; i++) {
		m_plain_text[i] = 0;
	}
